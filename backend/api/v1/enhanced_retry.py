    ...
    try:
        ...
        if not source_url:
            if "从B站下载:" in description:
                source_url = description.replace("从B站下载:", "").strip()
            elif "从YouTube下载:" in description:
                source_url = description.replace("从YouTube下载:", "").strip()
            elif "从抖音下载:" in description:
                source_url = description.replace("从抖音下载:", "").strip()

        if not source_url:
            raise HTTPException(status_code=400, detail="无法确定下载源")

        if "bilibili.com" in source_url or "b23.tv" in source_url:
            return await retry_bilibili_download(project_id, source_url, browser, db)
        if "youtube.com" in source_url or "youtu.be" in source_url:
            return await retry_youtube_download(project_id, source_url, browser, db)
        if "douyin.com" in source_url or "iesdouyin.com" in source_url:
            return await retry_douyin_download(project_id, source_url, browser, db)
...
async def retry_douyin_download(
    project_id: str,
    url: str,
    browser: Optional[str] = None,
    db: Session = Depends(get_db)
    ...
    try:
        ...
        logger.error(f"重试抖音下载失败: {e}")
        raise HTTPException(status_code=500, detail=f"重试抖音下载失败: {str(e)}")
