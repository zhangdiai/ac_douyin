...
def _pick_latest_file(files: list[Path]) -> Optional[Path]:
    """按修改时间选择最新文件。"""
    if not files:
        return None
...
async def parse_douyin_video(
    ...
    try:
        ...
        normalized_url = _extract_first_url(url)
        # 简单的URL验证
        ...
        info_dict = await loop.run_in_executor(None, extract_info_sync, normalized_url, ydl_opts)
        
        logger.info(f"抖音视频信息解析成功: {info_dict.get('title', 'Unknown')}")
...
async def create_douyin_download_task(request: DouyinDownloadRequest):
    """创建抖音视频下载任务 - 立即创建项目"""
    try:
        normalized_url = _extract_first_url(request.url)
        if not _is_douyin_url(normalized_url):
            raise HTTPException(status_code=400, detail="无效的抖音视频链接")
        request.url = normalized_url

        logger.info(f"创建抖音下载任务: {request.url}")
            ...
            if thumbnail_url:
                try:
                    ...
                    if response.status_code == 200:
                        ...
                        logger.info(f"抖音缩略图获取成功: {video_info.get('title', 'Unknown')}")
                    else:
                        logger.warning(f"下载抖音缩略图失败: {response.status_code}")
                except Exception as e:
                    logger.error(f"处理抖音缩略图失败: {e}")
                    # 缩略图处理失败不影响主流程
        ...
        logger.error(f"创建抖音下载任务失败: {str(e)}")
...
async def process_douyin_download_task(task_id: str, request: DouyinDownloadRequest, project_id: str):
    """处理抖音下载任务"""
    try:
        # 更新任务状态为处理中
        ...
        download_dir = data_dir / "temp" / f"douyin_{task_id}"
        ...
        # 查找下载的文件（仅在当前任务目录中搜索）
        video_candidates = list(download_dir.glob("*.mp4")) + list(download_dir.glob("*.mkv")) + list(download_dir.glob("*.webm"))
        subtitle_candidates = list(download_dir.glob("*.srt"))

        video_file = _pick_latest_file(video_candidates)
        subtitle_file = _pick_latest_file(subtitle_candidates)

        if not video_file:
            raise Exception("未找到下载的视频文件")

        video_path = str(video_file)
        subtitle_path = str(subtitle_file) if subtitle_file else ""
        ...
        if not subtitle_path:
            ...
            try:
                ...
                try:
                    subtitle_path = await _try_douyin_subtitle_strategies(request.url, download_dir, request.browser)
                    if subtitle_path:
        ...
        try:
            ...
            # 更新项目信息
            project.description = f"从抖音下载: {request.project_name}"
            ...
            if not srt_file_path.exists():
                ...
                logger.info(f"抖音下载任务失败: {task_id}, 项目ID: {project.id}, 原因: 字幕文件不存在")
                return
            ...
            logger.info(f"抖音下载任务完成: {task_id}, 项目ID: {project.id}")
            ...
            try:
                ...
                logger.info(f"抖音项目 {project.id} 下载完成，等待自动化流水线启动")
                ...
                if pipeline_result['status'] == 'started':
                    logger.info(f"抖音项目 {project.id} 自动化流水线已启动: {pipeline_result}")
                else:
                    logger.warning(f"抖音项目 {project.id} 自动化流水线启动结果: {pipeline_result}")
                
            except Exception as e:
                logger.error(f"启动抖音项目 {project.id} 自动化流水线失败: {str(e)}")
                # 即使处理启动失败，也要返回下载成功
                # 用户可以通过重试按钮重新启动处理
...
async def _try_douyin_subtitle_strategies(url: str, download_dir: Path, browser: Optional[str] = None) -> str:
    """尝试多种抖音字幕获取策略"""
    strategies = [
    ...
    for strategy in strategies:
        try:
            ...
            if subtitle_path:
                logger.info(f"抖音备用字幕策略成功")
                return subtitle_path
        except Exception as e:
            logger.warning(f"抖音备用字幕策略失败: {e}")
            continue
    
    logger.warning("所有抖音字幕获取策略都失败了")
    return ""
...
async def _try_extract_from_metadata(url: str, download_dir: Path, browser: Optional[str] = None) -> str:
    ...
    try:
        ...
        if subtitles or auto_subtitles:
            logger.info(f"发现抖音字幕信息: {list(subtitles.keys()) + list(auto_subtitles.keys())}")
