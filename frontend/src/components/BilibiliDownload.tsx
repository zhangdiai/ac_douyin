...
const BilibiliDownload: React.FC<BilibiliDownloadProps> = ({ onDownloadSuccess }) => {
  ...
  const validateVideoUrl = (url: string): boolean => {
    ...
    const douyinPatterns = [
      ...
      /^https?:\/\/iesdouyin\.com\/share\/video\/\d+/
    ]
  ...
  const normalizeInputUrl = (rawText: string): string => {
    if (!rawText) return ''
  ...
  const getVideoType = (url: string): 'bilibili' | 'youtube' | 'douyin' | null => {
    ...
    if (bilibiliPatterns.some(pattern => pattern.test(url))) {
      ...
      return 'youtube'
    } else if (douyinPatterns.some(pattern => pattern.test(url))) {
      return 'douyin'
    }
    return null
  }
  ...
  const parseVideoInfo = async () => {
    ...
    if (!videoType) {
      setError('请输入正确的B站/YouTube/抖音视频链接')
      return
    }
    ...
    try {
      ...
      if (videoType === 'bilibili') {
        response = await bilibiliApi.parseVideoInfo(normalizedUrl, selectedBrowser)
      } else if (videoType === 'youtube') {
        response = await bilibiliApi.parseYouTubeVideoInfo(normalizedUrl, selectedBrowser)
      } else {
        response = await bilibiliApi.parseDouyinVideoInfo(normalizedUrl, selectedBrowser)
      }
  ...
  const startPolling = (taskId: string, videoType: 'bilibili' | 'youtube' | 'douyin') => {
  ...
  const handleDownload = async () => {
    ...
    const videoType = getVideoType(normalizedUrl)
    if (!videoType) {
      message.error('请输入有效的B站/YouTube/抖音视频链接')
      return
    }
    ...
    try {
      ...
      if (response.project_id) {
        ...
        // 显示统一的成功提示
        const platformName = videoType === 'bilibili' ? 'B站' : videoType === 'youtube' ? 'YouTube' : '抖音'
    ...
    <div style={{
      ...
      <div style={{ marginBottom: '16px' }}>
        <Space direction="vertical" style={{ width: '100%' }} size={16}>
          <div>
            <Input.TextArea
              placeholder="请粘贴B站/YouTube/抖音视频链接或分享文案，支持：• B站：https://www.bilibili.com/video/BV1xx411c7mu • YouTube：https://www.youtube.com/watch?v=xxxxx • 抖音分享短链：https://v.douyin.com/xxxxx/"
              value={url}
              onChange={(e) => {
              ...
              onBlur={() => {
                // 失去焦点时自动解析
                const normalizedUrl = normalizeInputUrl(url.trim())
                if (normalizedUrl && !videoInfo && validateVideoUrl(normalizedUrl)) {
                  parseVideoInfo();
                }
              }}
              style={{
          ...
          {videoInfo && (
            <div style={{
              ...
              <Text style={{ color: 'rgba(255, 255, 255, 0.6)', fontSize: '12px' }}>
                {getVideoType(normalizeInputUrl(url)) === 'bilibili' ? 'UP主' : getVideoType(normalizeInputUrl(url)) === 'youtube' ? '频道' : '作者'}: {videoInfo.uploader || '未知'} • 时长: {videoInfo.duration ? `${Math.floor(videoInfo.duration / 60)}:${String(Math.floor(videoInfo.duration % 60)).padStart(2, '0')}` : '未知'}
              </Text>
            </div>
          )}
